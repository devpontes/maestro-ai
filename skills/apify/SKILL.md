...
- If an Actor returns no results: retry with broader search terms
- If an Actor fails: check if input parameters are valid
- If rate-limited: add delays between requests or reduce `maxItems`
